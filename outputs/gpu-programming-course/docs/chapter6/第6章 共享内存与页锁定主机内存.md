...
> <strong>注意</strong>：如果主机和设备使用<strong>统一地址空间（UVA）</strong>，且内存是通过 `cudaHostAlloc()` 分配的，那么主机指针和<strong>设备指针是同一个值</strong>。如果确定 UVA 已启用且内存被映射，但为保持代码的兼容性和清晰性，建议仍然调用 cudaHostGetDevicePointer()——该调用在 UVA 下也会成功，且不会产生额外开销。
